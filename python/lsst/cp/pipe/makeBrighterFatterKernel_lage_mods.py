...
class BrighterFatterKernel:
    ...
    def makeDetectorKernelFromAmpwiseKernels(self, detectorName, ampsToExclude=[], overwrite=False):
        ...
        # Lage - this can fail if index zero was a failed amp
        # modified clode to force it to use a good amp
        #avgKernel = np.zeros_like(list(self.ampwiseKernels.values())[0])

        for i,amp in enumerate(ampNames):
            if amp not in ampsToExclude:
                ampsToAverage.append(amp)
                goodAmpIndex = i

        avgKernel = np.zeros_like(list(self.ampwiseKernels.values())[goodAmpIndex])            
...
class MakeBrighterFatterKernelTask(pipeBase.CmdLineTask):
    ...
    def _calculateGains(self, dataRef, means, xcorrs):
        ...
        for amp in detector:
            ampName = amp.getName()
            # Lage - I added this because points into saturation were throwing off
            # the calculation.  Clearly there is a cleaner way to do this
            # It also throws out points which are far off a linear fit
            # We need to find the lowest mean value, because the points aren't necessarily in order.
            leastMean = 1.0E6
            ...
            for i in range(len(means[ampName])):
                ...
                var = xcorrs[ampName][i][0, 0] / 2.0
                if mean < 2.5 * leastMean:
                    ampMeans.append(mean)
                    ampVariances.append(var)
            if len(ampMeans) < 2:
                raise RuntimeError('We need at least two low level points to find the approximate gain')
            ptc_coefs = np.polyfit(ampMeans, ampVariances, 1)
            gain = 1.0 / ptc_coefs[0]
            # Now weed out points which are beyond saturation or far off the linear fit
            # These should probably be configurable parameters
            ...
            for i in range(len(means[ampName])):
                mean = (means[ampName][i][0] + means[ampName][i][1]) / 2.0
                var = xcorrs[ampName][i][0, 0] / 2.0
                if gain*mean < sat_level and var*gain > mean*(1-max_deviation) and var*gain < mean*(1+max_deviation):
                    ampMeans.append(mean)
                    ampVariances.append(var)
                else:
