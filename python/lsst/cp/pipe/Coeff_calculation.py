...
def xcorrFromVisit(butler, v1, v2, ccds=[1], n=5, border=10, plot=False, zmax=.04, fig=None, display=False,GAIN=None,sigma=5):
    ...
    mean = np.mean(means)
    xcorrImg,means1 = xcorr(*ims,Visits=[v1,v2], n=n, border=border, frame=len(ims) if display else None,CCD=[ccds[0]],GAIN=GAIN,sigma=sigma)
...
def isr(butler,v,ccd):
   ...
   # config.load(os.path.join(os.environ["OBS_SUBARU_DIR"], "config", "hsc", "isr.py"))
    ...
    config.crosstalk.value.coeffs.values = [0.0e-6, -125.0e-6, -149.0e-6, -156.0e-6,-124.0e-6, 0.0e-6, -132.0e-6, -157.0e-6,-171.0e-6, -134.0e-6,    0.0e-6, -153.0e-6,-157.0e-6, -151.0e-6, -137.0e-6,0.0e-6,]
...
def xcorr(im1, im2, Visits,n=5, border=20, frame=None,CCD=[1],GAIN=None,sigma=5,biasCorr=0.9241):
    ...
    for i, im in enumerate(ims):
        ...
        try:
            ...
            pass
       # im = trim(im, ccd)
        means[i] = afwMath.makeStatistics(im[border:-border,border:-border], afwMath.MEANCLIP,sctrl).getValue()
        temp=im.clone()
        ...
        for j,a in enumerate(ccd):
           ...
           # gain/=gain
            ...
            smi -= mean*gain
            smiTemp*=gain
        means1[i]=afwMath.makeStatistics(temp[border:-border,border:-border], afwMath.MEANCLIP,sctrl).getValue()
...
def gainInvest(butler, v1, v2, ccds=[12], n=5, border=10, plot=False, zmax=.05, fig=None, display=False,sigma=5,biasCorr=0.9241):
    ...
    diff[border:-border,border:-border] -= bkgd.getImageF(afwMath.Interpolate.CUBIC_SPLINE, afwMath.REDUCE_INTERP_ORDER)
    Var=[]
    CorVar=[]
        ...
        if COUNT%2==0:
            for i in range(1,func.shape[0]-1,2):
                for j in range(1,func.shape[0]-1,2):
                    resid[i,j]=float(func[i,j-1]+func[i,j+1]+func[i-1,j]+func[i+1,j]-4.0*func[i,j]-dx*dx*source[i-1,j-1])   
...
def xcorr_sim(im,im2,n=8, border=10,sigma=5):
    ...
    means1[1]=afwMath.makeStatistics(im2[border:-border, border:-border], afwMath.MEANCLIP,sctrl).getValue()
    im-=means1[0]
    im2-=means1[1]
    diff=im2.clone()
    diff-=im.clone()
...
def xcorr_bias(rangeMeans=[87500,70000,111000],repeats=5,sig=5,border=3,seed=None,nx=2000,ny=4000,case=0,a=.1):
        if seed is None:
            with open("/dev/random", 'rb') as file: local_random = np.random.RandomState(int(file.read(4).encode('hex'), 16))
        else:
