...
class ElectrostaticBrighterFatterSolveTask(pipeBase.PipelineTask):
    ...
    def run(self, inputPtc, camera, inputDims):
        ...
        bfCalib.rawMeans = inputPtc.rawMeans  # ADU
        bfCalib.rawVariances = inputPtc.rawVars  # ADU^2
        bfCalib.finalMeans = inputPtc.finalMeans  # ADU
        bfCalib.finalVariances = inputPtc.finalVars  # ADU^2
        bfCalib.expIdMask = inputPtc.expIdMask
        bfCalib.aMatrix = inputPtc.aMatrix
        bfCalib.aMatrixErr = inputPtc.ptcFitParsError

        # Use the PTC covariances as the cross-correlations.
        # The input covariances are in (x, y) index
        # ordering, as is the aMatrix.
        bfCalib.rawXcorrs = inputPtc.covariances  # ADU^2
        bfCalib.badAmps = inputPtc.badAmps
        maxFitRange = self.config.maxFitRange
        bfCalib.fitRange = maxFitRange
        bfCalib.gain = inputPtc.gain
        bfCalib.noise = inputPtc.noise
        bfCalib.valid = dict()
        bfCalib.updateMetadataFromExposures([inputPtc])
        ...
        for amp in detector:
            ...
            bfCalib.fitParamNames = fitParamNames
            bfCalib.freeFitParamNames = paramNames[np.array(fit.params._free)]
            bfCalib.fitParams[ampName] = fit.params
            bfCalib.chi2[ampName] = fit.getChi2()
            bfCalib.aMatrixModel[ampName] = fit.model()
            bfCalib.aMatrixSum[ampName] = symmetrize(fit.get_a()).sum()
            bfCalib.aMatrixModelSum[ampName] = symmetrize(fit.model()).sum()
            ...
            bfCalib.pixelDistortions[ampName] = pixelDistortions
