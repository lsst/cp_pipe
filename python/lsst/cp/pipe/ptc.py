...
class MeasurePhotonTransferCurveTask(pipeBase.CmdLineTask):
    ...
    **kwargs: `dict`
    ...
    def _getInitialGoodPoints(means, variances, maxDeviationPositive, maxDeviationNegative, minMeanRatioTest, minVarPivotSearch):
        ...
        ratioDeviations = [0.0 if a < minMeanRatioTest else (r/medianRatio)-1 for (a, r) in zip(means, ratios)]
    ...
    def fitPtc(self, dataset, ptcFitType):
        ...
        for i, ampName in enumerate(dataset.ampNames):
            ...
            print("meanVecOriginal", ampName, meanVecOriginal)
            print("varVecOriginal", ampName, varVecOriginal)
            ...
            mask = self._getInitialGoodPoints(meanVecOriginal, varVecOriginal,
                                              ...
                                              self.config.minMeanRatioTest, self.config.minVarPivotSearch)
            ...
            sigResids = (varVecFinal - ptcFunc(parsFit, meanVecFinal))/np.sqrt(varVecFinal)# Only for debug 
            print("After first fit", ampName)
            print("parsFit",parsFit)
            print("weightsY", weightsY)
            print("sigResids", sigResids)
            print("mask", mask)
